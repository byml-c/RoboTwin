...
class scan_object(Base_Task):
    ...
    def load_actors(self):
        ...
        if tag == 0:
            scanner_x_lim = [-0.3, -0.2]
            object_x_lim = [0.2, 0.3]
        else:
            scanner_x_lim = [0.2, 0.3]
            object_x_lim = [-0.3, -0.2]
        ...
        self.object_id = np.random.choice([0, 2, 4, 5], 1)[0]
        ...
        self.left_object_target_pose = [0.0, -0.06, 0.95, 0.707, 0, -0.707, 0]
        self.right_object_target_pose = [0.0, -0.06, 0.95, 0.707, 0, 0.707, 0]
    ...
    def play_once(self):
        ...
        self.move(
            self.move_by_displacement(arm_tag=scanner_arm_tag, x=0.05 if scanner_arm_tag == "right" else -0.05, z=0.1),
            self.move_by_displacement(arm_tag=object_arm_tag, x=0.05 if object_arm_tag == "right" else -0.05, z=0.1),
        )
